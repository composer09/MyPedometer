...
public class StepDetector implements SensorEventListener {
    ...
    private static List<StepListener> stepListenerList = new ArrayList<StepListener>();
    ...
    public void addStepListener(StepListener stepListener) {
        if (!stepListenerList.contains(stepListener)) stepListenerList.add(stepListener);
    }
     ...
     * 
     * @param stepListener
     *            StepListener to remove.
     */
    public void removeStepListener(StepListener stepListener) {
        if (stepListenerList.contains(stepListener)) stepListenerList.remove(stepListener);
    }
        ...
        if (direction == -mLastDirections[k]) {
            ...
            if (diff > mLimit) {
                ...
                if (isAlmostAsLargeAsPrevious && isPreviousLargeEnough && isNotContra) {
                    for (StepListener stepListener : stepListenerList) {
                        //StepService 에 구현된 onStep()실행
                        stepListener.onStep();
                    }
                    mLastMatch = extType;
                } else {
